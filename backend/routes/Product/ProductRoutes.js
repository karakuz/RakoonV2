...
router.get("/getRecommendation/:id", async (req, res) => {
  ...
  let similar_products = await db.get(`
  SELECT * 
  FROM items 
  WHERE items.category = '${product.category}' 
  ORDER BY RAND() 
  LIMIT 4`);
  ...
  const category = await db.get(`
    SELECT 
      category
    FROM items WHERE item_id = ${req.params.id}
  `);
  ...
  const rated_products = await db.get(`
    SELECT item_id, store.store_name, item_name, price, image, AVG(rate) as rate FROM 
      ...
      WHERE is_verified = 1 AND category = '${category[0].category}'
    `);
